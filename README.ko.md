...
```
에이전트 (Claude Code / Hermes / MCP 호환 클라이언트)
          │
          │  MCP stdio  (ontorag-mcp — 15개 툴)
          ▼
  ontorag-memory
  ├── EntityRegistry   free-text → canonical URI  (용어 노말라이즈)
  ├── AgentIdentity    user + workspace + session  (인스턴스 격리)
  ├── MemoryClient     remember / recall / find_path / summarize / …  (고수준 API)
...
| 다단계 경로 발견 | ✗ 불가능 | ✓ `find_path` / `find_path_transitive` |
| 결정 근거 추적 ("왜 X를 선택했나?") | ✗ 구조 없음 | ✓ `why()` + `rationale` 트리플 |
| 토큰 효율적 조회 | ✗ 파일 전체 읽기 | ✓ 쿼리 결과만 반환 |
| 다중 인스턴스 격리 | ✗ 디렉토리 공유 | ✓ user+workspace별 named graph |
| TTL / 만료 | ✗ 수동 | ✓ `prune(older_than_months=6)` |
| LLM 컨텍스트 주입 | ✗ 원문 붙여넣기 | ✓ `summarize()` → 단일 마크다운 호출 |
| 배치 저장 | ✗ 루프 + 오류 위험 | ✓ `remember_bulk([...])` |
...
`ontorag-mcp`는 15개의 메모리 툴을 MCP 프로토콜로 노출하는 stdio 엔트리포인트입니다.
MCP 호환 클라이언트라면 무엇이든 사용할 수 있습니다.

**Claude Desktop** — `~/.claude/claude_desktop_config.json`에 추가:
...
async with await MemoryClient.create() as mem:
    ...
    await mem.remember_bulk([
        {"subject": "urn:ag:incident:2026-06-27:api-timeout",
         "predicate": P.RATIONALE, "object": "Fuseki IPC JOIN 인덱스 누락"},
        {"subject": "urn:ag:incident:2026-06-27:api-timeout",
